 ...
 * @Last modified time: 2021-12-17T17:29:41+11:00
 *
 */
...
$(document).on('change', '.address_type', function(e) {
  ...
  if ($('option:selected', this).val() != 0) {
    ...
    $('.add_new_section').addClass('hide');
    if ($('option:selected', this).val() == 1 || $('option:selected', this)
      .val() == 3) {
      $('.postal_location_section').addClass('hide');
      $('.postal_location_row').removeClass('form-group');
      $('.ncl_type_section').addClass('hide');
...
$(document).on('change', '.postal_location', function(e) {
  if ($('option:selected', this).val() != 0) {
    ...
    var noncust_lodgement_loc_record = nlapiLoadRecord(
    ...
    var results_state = nlapiLookupField(
      'customrecord_ap_lodgment_location', corporate_office_internal_id,
      dropdown_fields, true);
    var results = nlapiLookupField('customrecord_ap_lodgment_location',
      corporate_office_internal_id, fields);
    ...
    var fullAddress = address2 + ',' + locality + ',' + state + ',' +
      zipcode;
    if (isNullorEmpty(lat) || isNullorEmpty(lng)) {
      $.ajax({
        ...
        url: 'https://maps.googleapis.com/maps/api/geocode/json?address=' +
          fullAddress +
          '&key=AIzaSyCa0FrFI1Y4k8fkD-x-Q5D1mzT2gifItOg&libraries=places',
        data: {},
        dataType: 'json',
        ...
        error: function() {
          alert(
            'Could not retrieve the Lat/Long for the selected Post Outlet'
          );
          return false;
        }
      });
...
$(document).on('click', '.remove_class', function(event) {
  ...
  if (confirm(
    ...
    if (!isNullorEmpty(addressid)) {
      ...
      var recCustomer = nlapiLoadRecord('customer', parseInt(
        nlapiGetFieldValue('custpage_customer_id')));

      for (indexY = 1; indexY <= recCustomer.getLineItemCount('addressbook'); indexY++) {
        if (parseInt(addressid) == parseInt(recCustomer.getLineItemValue(
            'addressbook', 'id', indexY))) {
...
$(document).on('click', '.edit_class', function(event) {
  ...
  var addressid = $(this).attr('data-addressid');
  var post_location = $(this).closest('tr').find('.postal_location1').attr(
    'data-postoutletid');
  ...
  var zip = $(this).closest('tr').find('.full_address').attr(
    'data-postcode');
  var no_service_address = $(this).closest('tr').find('.no_service_address')
    .attr('data-value');
...
$(document).on('click', '#add_address', function(e) {
  ...
  if (address_type == '2') {
    if ((isNullorEmpty(postal_location) && !($('input.default_shipping').is(
        ':checked'))) || isNullorEmpty(address1)) {
  ...
  if ($('input.default_billing').is(':checked')) {
    if (confirm(
        "You are attempting to edit a Billing address which is used for Invoices. Doing so will update how all future Invoices are addressed.\n\nAre you sure you want to continue editing the Billing address?"
      )) {} else {
      return false;
    }
  ...
  if (isNullorEmpty(address1) && isNullorEmpty(address2)) {
    ...
    var full_address = address1 + ', ' + address2 + ', ' + city + ', ' +
      state + ' - ' + zip;
  }
  ...
  if (isNullorEmpty(address_id)) {
    inlineQty +=
      '<tr><td class="first_col"><button class="btn btn-warning btn-sm edit_class glyphicon glyphicon-pencil" type="button" data-toggle="tooltip" data-addressid="' +
      null +
      '" data-placement="right" title="Edit"></button><br/><button class="btn btn-danger btn-sm remove_class glyphicon glyphicon-trash" type="button" data-toggle="tooltip" data-placement="right" data-addressid="' +
      id + '" title="Delete"></button></td>';

    // inlineQty += '<td><input type="text" class="form-control addr1" disabled value="' + addr1 + '"  /></td>';
    inlineQty += '<td><input disabled type="text" value="' +
      postal_location_text +
      '" class="form-control postal_location1" data-postoutletid="' +
      postal_location + '" />';
    ...
    if ($('input.default_out').is(':checked')) {
      inlineQty +=
        '<input disabled type="text" class="form-control no_service_address" value="Yes" data-value="1" style="text-align: center;"/>';
    } else {
      inlineQty +=
        '<input disabled type="text" class="form-control no_service_address" value="No" data-value="2" style="text-align: center;"/>';
    }
    inlineQty += '</td>';
    inlineQty +=
      '<td><input type="text" class="form-control full_address" disabled value="' +
      full_address + '" data-addressid="' + null + '" data-addr1="' +
      address1 + '" data-addr2="' + address2 + '" data-city="' + city +
      '" data-state="' + state + '" data-postcode="' + zip +
      '" data-mailingcode="' + mailingcode + '"/></td>';
    ...
    if ($('input.default_shipping').is(':checked')) {
      inlineQty +=
        '<input disabled type="text" class="form-control site" value="Yes" style="text-align: center;"/>';
    } else {
      inlineQty +=
        '<input disabled type="text" class="form-control site" value="No" style="text-align: center;"/>';
    }
    inlineQty += '</td>';
    inlineQty += '<td>';
    if ($('input.default_billing').is(':checked')) {
      inlineQty +=
        '<input disabled type="text" class="form-control billing" value="Yes" style="text-align: center;"/>';
    } else {
      inlineQty +=
        '<input disabled type="text" class="form-control billing" value="No"style="text-align: center;"/>';
    }
    inlineQty += '</td>';
    inlineQty += '<td>';
    if ($('input.default_residential').is(':checked')) {
      inlineQty +=
        '<input disabled type="text" class="form-control residential" value="Yes" style="text-align: center;"/>';
    } else {
      inlineQty +=
        '<input disabled type="text" class="form-control residential" value="No" style="text-align: center;"/>';
    }
    inlineQty += '</td>';
    inlineQty +=
      '<td><input type="text" class="form-control lat1" disabled value="' +
      lat + '"  /></td>';
    inlineQty +=
      '<td><input type="text" class="form-control lon1" disabled value="' +
      lng + '"  /></td>';
...
$(document).on('click', '#edit_address', function(e) {
  ...
  var postal_location_elem = document.getElementsByClassName(
    "postal_location1");
  var site_elem = document.getElementsByClassName("site");
  var billing_elem = document.getElementsByClassName("billing");
  var residential_elem = document.getElementsByClassName("residential");
  ...
  var no_service_address_elem = document.getElementsByClassName(
    "no_service_address");
  ...
  if (!isNullorEmpty(address_id)) {
    for (var i = 0; i < full_address_elem.length; ++i) {
      ...
      if (address_id == row_address_id) {
        ...
        postal_location_elem[i].setAttribute('data-postoutletid',
          postal_location);
        if ($('input.default_out').is(':checked')) {
        ...
        if (isNullorEmpty(address1) && isNullorEmpty(address2)) {
          ...
          var full_address = address2 + ', ' + city + ', ' + state + ' - ' +
            zip;
        } else if (!isNullorEmpty(address1) && isNullorEmpty(address2)) {
          var full_address = address1 + ', ' + city + ', ' + state + ' - ' +
            zip;
        } else {
          var full_address = address1 + ', ' + address2 + ', ' + city +
            ', ' + state + ' - ' + zip;
...
$(document).on('click', '#add_new', function(e) {
  ...
  newFilters[newFilters.length] = new nlobjSearchFilter(
    'custrecord_ap_lodgement_lat', null, 'equalto', lat);
  newFilters[newFilters.length] = new nlobjSearchFilter(
    'custrecord_ap_lodgement_long', null, 'equalto', lng);
  newFilters[newFilters.length] = new nlobjSearchFilter(
    'custrecord_noncust_location_type', null, 'is', ncl_type);
...
function updateAddress(submit) {
  var recCustomer = nlapiLoadRecord('customer', parseInt(nlapiGetFieldValue(
    'custpage_customer_id')));
    ...
    if (edit_class_elem.length > 0) {
      for (var i = 0; i < edit_class_elem.length; ++i) {
        ...
        if (isNullorEmpty(row_address_id)) {
          ...
          var lineIndex = recCustomer.findLineItemValue('addressbook',
            'internalid', row_address_id);
          if (lineIndex > 0) {
        ...
        if (site_elem[i].value == 'Yes') {
          recCustomer.setCurrentLineItemValue('addressbook', 'label',
            'Site Address');
        ...
        recCustomer.setCurrentLineItemValue('addressbook', 'addressee',
          recCustomer.getFieldValue('companyname'));
        ...
        if (site_elem[i].value == 'Yes') {
          recCustomer.setCurrentLineItemValue('addressbook', 'defaultshipping',
            'T');
        } else {
          recCustomer.setCurrentLineItemValue('addressbook', 'defaultshipping',
            'F');
        ...
        if (billing_elem[i].value == 'Yes') {
          recCustomer.setCurrentLineItemValue('addressbook', 'defaultbilling',
            'T');
        } else {
          recCustomer.setCurrentLineItemValue('addressbook', 'defaultbilling',
            'F');
        ...
        if (residential_elem[i].value == 'Yes') {
          recCustomer.setCurrentLineItemValue('addressbook', 'isresidential',
            'T');
        } else {
          recCustomer.setCurrentLineItemValue('addressbook', 'isresidential',
            'F');
        ...
        var postal_location_id = postal_location_elem[i].getAttribute(
          'data-postoutletid')
        if (outofstate == 'No') {
          out_of_region[(i + 1)] = 2;
        } else {
        ...
        if (!isNullorEmpty(corporate_office_internal_id) &&
          corporate_office_internal_id != 0) {
      ...
      alert(
        'You will need to provide complete address details before submitting.\n\nPlease review and provide complete address(es). \n Please click Add /Edit to add the address before clicking Submit'
      );
      return false;
    }
  } else {
...
function onclick_back() {
  ...
  var upload_url = baseURL + nlapiResolveURL('SUITELET', nlapiGetFieldValue(
      'custpage_suitlet'), nlapiGetFieldValue('custpage_deploy')) +
