 ...
 * @Last Modified time: 2020-03-12 12:07:24
 *
 */
...
function address_creation(request, response) {
    ...
    if (request.getMethod() === "GET") {
        ...
        if (role == 1000) {
            ...
            params_inner = JSON.parse(request.getParameter('entry_string'));
            nlapiLogExecution('DEBUG', 'params', params_inner);
            var script_id = (params_inner.id);
            var deploy_id = (params_inner.deploy);
            if(script_id == 'customscript_sl_lead_capture'){
